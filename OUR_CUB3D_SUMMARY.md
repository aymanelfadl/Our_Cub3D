...
## Testing & Runtime
- Regression tests and the Valgrind leak check cover both happy and error paths. At runtime, `./cub3D maps/good/hanzo.cub` or `maps/good/dungeon.cub` opens a window, renders textured walls, and supports WASD rotation/translation (`Q`/`E` strafe, `Esc` exit).
